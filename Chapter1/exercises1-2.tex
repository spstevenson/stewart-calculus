...
\end{displaymath} 
		...
		\item If $f(x) = x + 4$ and $h(x) = 4x - 1$, find a function \emph{g} such that $g \circ f = h$.
		
			$$g(x) = 4x -17$$
			$$4(x + 4) - 17 = 4x - 1$$
			
		\item Suppose \emph{g} is an even function and let $h = f \circ g$. Is \emph{h} always an even function?
		...
		\item Suppose \emph{g} is an odd function and let $h = f \circ g$. Is $h$ always an odd function? What if
			\emph{f} is odd? What if \emph{f} is even?
			
			$$g(-x) = -g(x)$$
			$$h(-x) = f(-g(x))$$
			
			If \emph{f} is also odd then \emph{h} will be odd. If \emph{f} is even \emph{h} will be even.
	\end{enumerate}
\end{document}